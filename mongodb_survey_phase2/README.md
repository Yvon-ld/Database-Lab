...
### 10.2 已完成浏览器级手工检查
已完成一轮浏览器级检查（基于 Playwright 自动化），确认以下行为：
...
## 11. 说明与限制
- 当前项目采用服务端渲染页面 + 表单提交，不是独立 REST 接口平台
- 匿名重复提交限制基于会话（`session`），更换浏览器或清理会话后可再次提交
